    ...
    public T remove(T value){
        ...
        while(dataRoot!=null){
            ...
            if(cmp<0){
                ...
                if(dataRoot.getRight()!=null){  //被删除节点的右子树存在时
                    RBTreeNode<T> min = removeMin(dataRoot.getRight());  //min是右子树中值最小的那个节点
                    //x used for fix color balance
                    //若min存在右子树，则x为min的右子树，否则x为min的父节点
                    RBTreeNode<T> x = min.getRight()==null ? min.getParent() : min.getRight();
                    boolean isParent = min.getRight()==null;    //标志x是否是父节点

                    min.setLeft(dataRoot.getLeft());  //将被删除节点的左子树设为min的左子树
                    setParent(dataRoot.getLeft(),min);   //将min设为被删除节点左子树的父节点
                    if(parent.getLeft()==dataRoot){   //当前被删除节点是其父节点的左子树，则将min设为其父节点的左子树
                        parent.setLeft(min);
                    }else{                             //否则将min设为右子树
                        parent.setRight(min);
                    }
                    ...
                    boolean curMinIsBlack = min.isBlack();     //记录min节点的颜色
                    //inherit dataRoot's color
                    min.setRed(dataRoot.isRed());         //将min的颜色染为被删除节点的颜色

                    //如果min并非被删除节点的右儿子，则让min与被删除节点的右儿子建立父子关系
                    if(min!=dataRoot.getRight()){
                        min.setRight(dataRoot.getRight());
                        setParent(dataRoot.getRight(),min);
                    }
                    /**以上几步的目的均是为了用min替代被删除节点在红黑树中的位置 **/


                    //如果min是黑节点，则其所在路径少了一个黑节点，需要进行修复
                    ...
                    setParent(dataRoot.getLeft(),parent);   //直接让被删除节点的父节点成为被删除节点左儿子的父亲
                    if(parent.getLeft()==dataRoot){       //如果被删除节点是左儿子，则让被删除节点的左儿子成为其父节点的左儿子，反之则成为右儿子
